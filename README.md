...
| `CORS_ORIGINS` | browser origins allowed to call `/api/v1` (comma-separated, or `*`) |
...
> **[ENDPOINTS.md](ENDPOINTS.md)** has every endpoint with a copy-pasteable
> request (curl + PowerShell) and its real response.

## Public integration API (`/api/v1`)

One answer, three renderings — use whichever fits the app you're integrating.
All accept **POST** (JSON body) or **GET** (query string) with the same fields:
`threadId` (optional — omit it and each request is its own conversation) and
`message`.

| Endpoint | Content type | Use it for |
| --- | --- | --- |
| `/api/v1/chat` | `application/json` | normal API integration |
| `/api/v1/chat/raw` | `text/plain` | the reply text only — SMS, WhatsApp, a webhook |
| `/api/v1/chat/html` | `text/html` | drop-in markup: message + product cards |
...
```json
{
  "success": true,
  "threadId": "cust-42",
  "message": "We have a pink baby towel that fits your budget.\n\n1. Soft Baby Towel – Rs 399",
  "products": [
    {
...
```bash
# Raw text — headers carry the thread and product count
curl "localhost:3000/api/v1/chat/raw?threadId=cust-42&message=any%20discounts"
```
```
Here's a pink baby towel within your budget:

1. Soft Baby Towel – Premium Cotton Absorbent Kids Bath Towel – Rs 399
```
`x-thread-id` and `x-product-count` come back as response headers.

```bash
# HTML fragment (add ?full=1 for a standalone document / iframe)
curl "localhost:3000/api/v1/chat/html?threadId=cust-42&message=pink%20towel"
```
```html
<div class="sai-response">
  <div class="sai-message"><p>Here's a pink baby towel that fits your budget.</p></div>
  <ol class="sai-products">
    <li class="sai-product" data-product-id="gid://shopify/Product/8235448008751">
      <span class="sai-product-index">1</span>
      <img class="sai-product-image" src="https://cdn.shopify.com/…" alt="Soft Baby Towel" loading="lazy" />
      <div class="sai-product-body">
        <h3 class="sai-product-title">Soft Baby Towel …</h3>
        <ul class="sai-variants">
          <li class="sai-variant">
            <span class="sai-variant-title">Pink / 30*30 Inches</span>
            <span class="sai-variant-price">Rs 399</span>
            <span class="sai-variant-stock is-in">In stock</span>
          </li>
        </ul>
        <a class="sai-product-link" href="https://your-store.myshopify.com/products/…">View product</a>
      </div>
    </li>
  </ol>
</div>
...
The HTML is generated by the backend, never by the LLM. Class names are
`sai-`-prefixed and unstyled in fragment mode, so the host page styles them;
product links become absolute using `SHOPIFY_STORE_DOMAIN`. Every value is
escaped — model text and Shopify data alike — and `javascript:`/`data:` URLs
are dropped rather than emitted.

Also under v1, without any LLM: `GET /api/v1/products/search` (same filters as
below) and `GET /api/v1/discounts`.

Notes for integrators:
- Errors keep the endpoint's content type: `/raw` fails as plain text, `/html`
  as an error fragment, `/chat` as JSON — a text/plain client never suddenly
  gets JSON.
- Responses are clean by default; append `?debug=true` for the diagnostics
  block.
- Browser callers need `CORS_ORIGINS` set (comma-separated, or `*`).
- Chat is rate limited to 20 requests/minute per IP.
...
70 tests, no network access (the Shopify transport is stubbed):
...
- HTML rendering: markup injection from model text *and* from Shopify data,
  `javascript:`/`data:` URLs dropped, absolute vs relative product links,
  empty-product answers, and the standalone page
